 ...
 *    files the node under `incomplete`. This page runs on gradients: the
 *    combiner diagram flows a `--accent → --accent-4` linear gradient down its
 *    connector paths, the entropy meter fill is a gradient, the handshake bars
 *    are gradient-toned, and several panels wash a faint gradient behind copy.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The `.reveal` sections ramp
 *    `opacity: 0 → 1`, and the disabled benchmark button dims its subtree.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The combiner's
 *    session-key `.mono-block`, the code-panel `.code-block`, the handshake
 *    rows and the bit-grid pair all live in `overflow-x: auto` wraps that clip
 *    at 380px. Content scrolled past the client box is not dimmed or partly
 ...
 *    inventing a failure. Skip it, and rely on the wider viewport where the
 *    very same element is visible and measured for real.
 *
 ...
 *    combiner diagram is an `<svg>`: it draws each component box as a `<rect>`
 *    and then its label as a following `<text>`, so a label's backdrop is a
 *    PRECEDING SIBLING shape, not an ancestor. The `SHA-256` combiner label is
 *    `--mono-block-fg` on a solid `--mono-block-bg` rect; an ancestor-only walk
 *    would composite it onto the svg's own transparent background and report a
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
    ...
    for (const el of Array.from(document.querySelectorAll('body *'))) {
      ...
      const cs = styleOf(el);
      // SVG text takes its ink from `fill`, not `color`. The combiner diagram's
      // `<text>` labels are painted in `fill: var(--ink-strong)` /
      // `var(--ink-soft)` / `var(--mono-block-fg)`, and reading `color` there
