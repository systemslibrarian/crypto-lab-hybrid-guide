 ...
 *
 * On this page the `.reveal` sections ramp `opacity: 0 → 1` on scroll, and the
 * reduced-motion block restores them with an explicit `.reveal { opacity: 1;
 * transform: none }` rather than merely cancelling the transition — so nothing
 * goes blank under the preference. That is a property of the stylesheet as it
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#app .cl-hero-title')).toHaveText('Hybrid KEM');
  await expect(page.locator('#app main.lab-main > .lab-section')).toHaveCount(7);
  await expect(page.locator('#playground-heading')).toHaveText('The Combiner');
 ...
 * plausible offender: a 64-hex session key, the code-panel snippet, the
 * handshake byte bars and the bit-grid pair are all wide, monospace, or
 * `min-width: max-content`. Each lives inside its own `overflow-x: auto`
 * container, which is the correct answer — but a container missed, or a long
 * string outside one, pushes the document sideways.
 */
 ...
 * content actually overflows — several of this page's scrolling regions only
 * overflow at phone width, so a desktop-only gate never saw them. This
