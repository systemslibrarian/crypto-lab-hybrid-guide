...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // PER SIDE, not `border-top` for all four. The form that stood here read
      // `border-*-width` on every side but then asked `borderTopStyle` alone
      // whether a border existed at all, and measured `borderTopColor` alone
      // below — so a control delineated by ONE side is judged on the wrong
      // edge. `border-bottom: 3px solid` with `border-top: none` reads as
      // "no border" and the real delineator is never measured; the mirror case
      // measures a `border-top` that is `0px` wide against a control whose
      // visible edge is elsewhere. Elsewhere in this fleet that reported
      ...
      // transparent spacer as a border makes a control with no fill and no
      // painted edge, identified by its text alone and therefore exactly the
      // case the "is it trying to draw itself as a control?" test below exists
      // to exclude, report 1.00:1.
